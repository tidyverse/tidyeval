...
The first chapter [Why and how](#why-how) provides the motivation for tidy eval, presents the problems that it poses in day-to-day programming, and the general theory and tools for solving those. If you are in a hurry, you can jump straight to [Do you need tidy eval?](do-you-need). A lot can be done without writing a single line of tidy eval! If you are positive you need it to solve your problem, [Getting up to speed](#up-to-speed) is a self-contained chapter that will teach you the basic workflow of wrapping a tidyverse pipeline in a reusable function. The book ends with a series of recipes and idioms for solving various dplyr and ggplot2 problems.
...
Changing the context of evaluation is useful for three main purposes:
...
Compare to the equivalent subsetting code where it is necessary to be explicit about where the columns come from:

```{r, eval = FALSE}
index <-
  !is.na(starwars$height) & !is.na(starwars$gender) &
  starwars$height < 200 & starwars$gender == "male"

starwars[index, ]
```

Data masking is natural for many users because it reduces boilerplate and results in code that maps more directly to how they think about data manipulation problems. This is why many R beginners are tempted to use `attach()`. This function attaches a data frame to the search path in much the same way that `library()` attaches exported objects of a package to make it easier to use interactively. However `attach()` is not a recommended way of using a data mask for the same reason that global options are frown on — it makes your code less reproducible because of the permanent scope of these effects. On the other hand, data masking functions have only temporary effects, making them a good choice for both interactive and programmatic uses.

Data masking is only possible because R allows suspending the normal flow of evaluation. If code was evaluated in the normal way, R would not be able to find the relevant columns for the computation:
...
[^3]: As you can see, blueprints are also called **quosures**. These are special types of [expressions](#glossary-expr) that keep track of the current context, or [environment](#glossary-env).
...
To sum up, the distinctive look and feel of data masking UIs requires suspending the normal evaluation of R code. Once captured as quoted code, it can be resumed in a different context. Unfortunately, the delaying of code has a dark side. While it is straightforward to substitute values, it is harder to substitute column names in delayed code. Making indirect references to columns requires learning a bit of new theory and some new tools.


## Unquoting code

Data masking functions prevent the normal evaluation of their arguments by quoting them. Once in possession of the blueprints of their arguments, they create a data mask and resume evaluation in this new context. The flip side of delaying code in this way is that making indirect references to columns is only possible by modifying the quoted code before it gets evaluated. We need a surgery operator for blueprints of R code. This is what the `!!` operator[^4] is all about.

[^4]: Pronounced bang bang.

In the world of normal functions, making indirect references is easy. Expressions that yield the same values can be freely interchanged, a property that is sometimes called [referential transparency](https://en.wikipedia.org/wiki/Referential_transparency):
...
As you can observe, `!!` modifies the quoted code by inlining the value of its operand right into the blueprint. What would it take to create an indirect reference to a column name? Passing its name as a string will not produce what you expect:
...
Symbols are a special type of string that represent other objects. When a piece of R code is evaluated, every bare variable name is actually a symbol that represent some value as defined in the current context. Let's see what the modified blueprint looks like when we inline a symbol:
...
# Do you need tidy eval {#do-you-need}
...
* Scoped variants.
* Passing dots.
* Iterating over list columns


# Getting up to speed {#up-to-speed}
