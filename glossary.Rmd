...
This glossary contains all the vocabulary necessary to work with tidy evaluation and, more generally, with [expressions](#glossary-expr). The definitions in rlang are generally consistent with base R. When they differ, both definitions are presented so you can navigate between these two worlds more easily.
...
An expression is a piece of R code that describes a value:
...
Expressions are normally transient. They are executed ([evaluated](#glossary-eval)) when you source a file or call a function and you only see their final value or their side effects (such as console output). However in R you can suspend the normal evaluation of expressions with the [quotation](#glossary-quotation) mechanism:
...
In base R, "expression" refers to a special type of vectors that contains quoted expressions in the rlang sense:
...
The only advantage of expression vectors compared to lists is that they include **source references**. The most apparent manifestation of source references is that expression vectors are printed with whitespace and comments preserved:
...
Constants, also called "literals", always have the same value no matter the context. On the other hand symbols and calls are [symbolic](#glossary-constant-symbolic) objects: their value depends on an [environment](#glossary-env) and what kind of objects are defined there.
...
On the other [symbols](#glossary-sym) depend on current definitions:
...
This works fine but what if the data frame contains a column called `factor`? The expression will be evaluated with the wrong definition:
